...
class BusStop(object):
    ...
    def retrieve_scraper(self):
        ...
        for bus in buses:
            ...
            r.set('bus:%d:minutes_to_departure' % (k),
                  bus['minutes_to_departure'])
    ...
    def retrieve_redis(self):
        """
        retrieve a list of buses from redis for an atco list that exists.
        ...
        for l in range(i):
            ...
            bus['minutes_to_departure'] = r.get('bus:%s:'
    ...
    def retrieve_buses(self):
        ...
        check if there's a currently cached set of buses in redis.
            ...
            if not, get the data using busscraper.stop, and put it in redis
        ...
        # if we've not already got it in redis, get from oxontime, put in redis
        else:
...
def name_joiner(bus_stop):
    """
    passed a bus stop, returns a sensible name for it.
    Based on common name and landmark
    """
...
def get_stops(latitude, longitude, offset):
    ...
    stop_list = query_db('SELECT * FROM '
                         '(SELECT AtcoCode, CommonName, Landmark,'
                         '(((latitude - ?) * (latitude - ?)) + '
                         '(longitude - (?)) * (longitude - (?))) * '
                         '(110 * 110) AS dist FROM stops ORDER BY dist ASC) '
                         'AS tab WHERE tab.dist <= (1 * 1);',
                         [latitude, latitude, longitude, longitude])

    # use the offset to figure out which stops in the list we're interested in.
    # if the list has more stops than we're using, set more variable to be true
    ...
    for bus_stop in stop_list:
        ...
        v = BusStop(bus_stop["AtcoCode"], name=name_joiner(bus_stop),
                    distance=int(bus_stop["dist"] * 1000))

        options.append(v.__dict__)

    # return a tuple containing the list of stop options
    # and indicating whether or not there are more stops
    return (options, more)
...
def get_stop(stop_id):
    ...
    bus_stop = query_db('SELECT * FROM stops WHERE AtcoCode = ?;',
                        [stop_id])[0]

    return BusStop(bus_stop["AtcoCode"],
                   name=name_joiner(bus_stop),
